        ...
        async with self.genshin_helper.genshin(user_id, region, player_id, offset) as client:
            ...
            try:
                ...
                if stoken is not None:
                    ...
                    try:
                        qr = await client.gen_login_qrcode(app_id)
                        logger.success("用户 %s 尝试登录 成功", user_id)
                        await client.accept_login_qrcode(qr[0])
                        logger.success("用户 %s 同意登录 成功", user_id)
                        game_token, _ = await client.check_login_qrcode(qr[1], app_id)
                        logger.success("用户 %s 获取 game_token 成功", user_id)
                        token = await client.login_game_by_game_token(game_token=game_token["token"])
