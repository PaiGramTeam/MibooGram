...
from core.services.history_data.models import HistoryDataChallengeHolo
from core.services.history_data.services import HistoryDataChallengeHoloServices
...
cmd_pattern = r"(?i)^/challenge_holo(?:@[\w]+)?\s*((?:\d+)|(?:all))?\s*(pre)?"
msg_pattern = r"^拟境湮灭战数据((?:查询)|(?:总览))(上期)?\D?(\d*)?.*?$"
MAX_FLOOR = 3
MAX_STARS = MAX_FLOOR * 4
...
class ChallengeHoloPlugin(Plugin):
    """拟境湮灭战数据查询"""
    ...
    def __init__(
        ...
        history_data_abyss: HistoryDataChallengeHoloServices,
        redis: RedisDB,
    ):
        ...
        self.cache = RedisCache(redis.client, key="plugin:challenge_holo:history")
    ...
    @handler.command("challenge_holo", block=False)
    ...
    async def command_start(self, update: Update, context: CallbackContext) -> None:
        ...
        if (message.text.startswith("/") and "help" in message.text) or "帮助" in message.text:
            await message.reply_text(
                "<b>拟境湮灭战数据</b>功能使用帮助（中括号表示可选参数）\n\n"
                "指令格式：\n<code>/challenge_holo + [层数/all] + [pre]</code>\n（<code>pre</code>表示上期）\n\n"
                "文本格式：\n<code>拟境湮灭战数据 + 查询/总览 + [上期] + [层数]</code> \n\n"
                "例如以下指令都正确：\n"
                "<code>/challenge_holo</code>\n<code>/challenge 1 pre</code>\n<code>/challenge all pre</code>\n"
                "<code>拟境湮灭战数据查询</code>\n<code>拟境湮灭战数据查询上期第1层</code>\n<code>拟境湮灭战数据总览上期</code>",
                parse_mode=ParseMode.HTML,
            )
            self.log_user(update, logger.info, "查询[bold]拟境湮灭战数据[/bold]帮助", extra={"markup": True})
            return
        ...
        self.log_user(
            update,
            logger.info,
            "[bold]拟境湮灭战挑战数据[/bold]请求: uid=%s previous=%s",
            uid,
            previous,
            extra={"markup": True},
        )
        ...
        try:
            async with self.helper.genshin_or_public(user_id, uid=uid) as client:
                reply_text = await message.reply_text(
                    f"{config.notice.bot_name} 需要时间整理拟境湮灭战数据，还请耐心等待哦~"
                )
            ...
            return
        except AbyssUnlocked:  # 若拟境湮灭战未解锁
            await reply_message_func("还未解锁拟境湮灭战哦~")
            return
        except AbyssFastPassed:  # 若拟境湮灭战已快速通过
            await reply_message_func("本层已被快速通过，无详细数据~")
            return
        except IndexError:  # 若拟境湮灭战为挑战此层
        ...
        self.log_user(update, logger.info, "[bold]拟境湮灭战挑战数据[/bold]: 成功发送图片", extra={"markup": True})

    async def get_rendered_pic_data(self, client: "ZZZClient", uid: int, previous: bool) -> "ZZZHoloBossDetail":
        abyss_data = await client.get_zzz_challenge_holo(uid, previous=previous, lang="zh-cn")
    ...
    async def get_rendered_pic(  # skipcq: PY-R1000 #
        ...
        Args:
            abyss_data (ZZZChallenge): 拟境湮灭战数据
            uid (int): 需要查询的 uid
        ...
        if not abyss_data.has_data or not abyss_data.list:
            raise AbyssUnlocked()
        start_time = abyss_data.start_time.datetime.strftime("%m月%d日 %H:%M")
        ...
        for floor_data in abyss_data.list:
            ...
            floors.append(
                {
                    "floor": floor_data,
                    "floor_time": f"{t.minute} 分 {t.second} 秒",
                }
            )
    ...
    async def save_abyss_data(
        history_data_abyss: "HistoryDataChallengeHoloServices", uid: int, abyss_data: "ZZZHoloBossDetail"
    ) -> bool:
    ...
    @staticmethod
    def get_season_data_name(data: "HistoryDataChallengeHolo"):
        last_battles = data.abyss_data.list[0]
        start_time = data.abyss_data.start_time.datetime
        time = start_time.strftime("%Y.%m.%d")
        name = ""
        honor = ""
        if data.abyss_data.total_star == MAX_STARS:
            honor = "👑"
            num_of_characters = len(last_battles.avatar_list)
            if num_of_characters == 2:
                honor = "双通"
            elif num_of_characters == 1:
                honor = "单通"

        return f"{name} {time} {data.abyss_data.total_star} ★ {honor}".strip()
    ...
    async def get_session_button_data(self, user_id: int, uid: int, force: bool = False):
        ...
        abyss_data = [HistoryDataChallengeHolo.from_data(i) for i in data]
        buttons = [
            {
                "name": self.get_season_data_name(abyss_data[idx]),
                "value": f"get_challenge_holo_history|{user_id}|{uid}|{value.id}",
            }
    ...
    async def gen_season_button(
        ...
        if last_page:
            last_button.append(
                InlineKeyboardButton(
                    "<< 上一页",
                    callback_data=f"get_challenge_holo_history|{user_id}|{uid}|p_{last_page}",
                )
            )
        if last_page or next_page:
            last_button.append(
                InlineKeyboardButton(
                    f"{page}/{all_page}",
                    callback_data=f"get_challenge_holo_history|{user_id}|{uid}|empty_data",
                )
            )
        if next_page:
            last_button.append(
                InlineKeyboardButton(
                    "下一页 >>",
                    callback_data=f"get_challenge_holo_history|{user_id}|{uid}|p_{next_page}",
                )
            )
        if last_button:
    ...
    @handler.command("challenge_holo_history", block=False)
    @handler.message(filters.Regex(r"^拟境湮灭战历史数据"), block=False)
    async def abyss_history_command_start(self, update: Update, _: CallbackContext) -> None:
        user_id = await self.get_real_user_id(update)
        message = update.effective_message
        uid, offset = self.get_real_uid_or_offset(update)
        uid: int = await self.get_uid(user_id, message.reply_to_message, uid, offset)
        self.log_user(update, logger.info, "查询拟境湮灭战历史数据 uid[%s]", uid)
        ...
        async with self.helper.genshin_or_public(user_id, uid=uid) as _:
            ...
            if not buttons:
                await message.reply_text("还没有拟境湮灭战历史数据哦~")
                return
        await message.reply_text("请选择要查询的拟境湮灭战历史数据", reply_markup=InlineKeyboardMarkup(buttons))

    async def get_challenge_holo_history_page(self, update: "Update", user_id: int, uid: int, result: str):
        """翻页处理"""
        callback_query = update.callback_query

        self.log_user(update, logger.info, "切换拟境湮灭战历史数据页 page[%s]", result)
        ...
        async with self.helper.genshin_or_public(user_id) as _:
            ...
            if not buttons:
                await callback_query.answer("还没有拟境湮灭战历史数据哦~", show_alert=True)
                await callback_query.edit_message_text("还没有拟境湮灭战历史数据哦~")
                return
        await callback_query.edit_message_reply_markup(reply_markup=InlineKeyboardMarkup(buttons))
        await callback_query.answer(f"已切换到第 {page} 页", show_alert=False)

    async def get_challenge_holo_history_floor(self, update: "Update", data_id: int):
        """渲染层数数据"""
        callback_query = update.callback_query
        ...
        if not data:
            ...
            return
        abyss_data = HistoryDataChallengeHolo.from_data(data)
    ...
    @handler.callback_query(pattern=r"^get_challenge_holo_history\|", block=False)
    async def get_challenge_holo_history(self, update: "Update", _: "ContextTypes.DEFAULT_TYPE") -> None:
        callback_query = update.callback_query
        user = callback_query.from_user

        async def get_challenge_holo_history_callback(
        ...
        result, user_id, uid = await get_challenge_holo_history_callback(callback_query.data)
        if user.id != user_id:
        ...
        if result.startswith("p_"):
            await self.get_challenge_holo_history_page(update, user_id, uid, result)
            return
        data_id = int(result)
        await self.get_challenge_holo_history_floor(update, data_id)
    ...
    async def abyss_use_by_inline(self, update: "Update", context: "ContextTypes.DEFAULT_TYPE", previous: bool):
        ...
        self.log_user(update, logger.info, "查询拟境湮灭战挑战总览数据 previous[%s]", previous)
        notice = None
        try:
            async with self.helper.genshin_or_public(user_id, uid=uid) as client:
